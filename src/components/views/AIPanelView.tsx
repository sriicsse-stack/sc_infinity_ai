  ...
  const renderInlineFormat = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={i} className="font-bold text-white dark:text-white light:text-slate-900">{part.slice(2, -2)}</strong>;
      }
      if (part.startsWith('`') && part.endsWith('`')) {
        return <code key={i} className="px-1.5 py-0.5 rounded bg-slate-800 dark:bg-slate-800 light:bg-slate-200 text-indigo-300 dark:text-indigo-300 light:text-indigo-700 font-mono text-[10px]">{part.slice(1, -1)}</code>;
      }
      return part;
    });
  };
    ...
    return (
      <div className="space-y-1.5 leading-relaxed">
        {lines.map((line, lIdx) => {
          ...
          if (line.startsWith('### ')) {
            return (
              <h4 key={lIdx} className="text-xs font-bold text-indigo-300 dark:text-indigo-300 light:text-indigo-600 mt-2.5 mb-1 flex items-center gap-1.5">
          ...
          if (line.startsWith('## ')) {
            return (
              <h3 key={lIdx} className="text-sm font-extrabold text-white dark:text-white light:text-slate-900 mt-3 mb-1.5">
                {line.replace('## ', '')}
              </h3>
            );
          }
          ...
          if (line.trim().startsWith('• ') || line.trim().startsWith('- ')) {
            ...
            return (
              <div key={lIdx} className="flex items-start space-x-2 pl-1 text-[11px] text-slate-300 dark:text-slate-300 light:text-slate-700">
                <span className="text-indigo-400 mt-0.5">•</span>
                <div>{renderInlineFormat(itemText)}</div>
              </div>
            );
          }

          return <div key={lIdx} className="text-xs">{renderInlineFormat(line)}</div>;
        })}
      </div>
    );
  };
    ...
    <div className="w-80 sm:w-96 h-full bg-[#0a0d15] dark:bg-[#0a0d15] light:bg-slate-50 border-l border-[#1f293d] dark:border-[#1f293d] light:border-slate-200 flex flex-col text-xs select-none relative">
      ...
      <div className="h-10 px-3.5 border-b border-[#1f293d] dark:border-[#1f293d] light:border-slate-200 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-1.5">
          {/* API Key Status Pill / Config */}
          <button
            onClick={() => setIsKeyModalOpen(true)}
            className={`px-2 py-0.8 rounded-lg border text-[11px] flex items-center space-x-1 ${
              geminiApiKey 
                ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' 
                : 'bg-[#121826] border-slate-700/60 text-slate-400 hover:text-white'
            }`}
            title={geminiApiKey ? "Gemini API Key Connected (BYOK)" : "Set Custom Gemini API Key"}
          >
            <Key className="w-3 h-3" />
            <span>{geminiApiKey ? 'BYOK' : 'API Key'}</span>
          </button>
          ...
          <div className="relative">
            ...
            {isModelDropdownOpen && (
              <div className="absolute top-full right-0 mt-1 w-48 bg-[#0e1320] dark:bg-[#0e1320] light:bg-white rounded-xl border border-slate-800 dark:border-slate-800 light:border-slate-200 shadow-2xl py-1 z-50">
                {(['gemini-2.0-flash', 'gemini-1.5-pro', 'gemini-1.5-flash', 'gemini-pro'] as AIModelType[]).map((m) => (
                  <button
                    key={m}
                    onClick={() => {
                      setModel(m);
                  ...
                  >
                    <span>{m === 'gemini-2.0-flash' ? 'Gemini 2.0 Flash' : m === 'gemini-1.5-pro' ? 'Gemini 1.5 Pro' : m === 'gemini-1.5-flash' ? 'Gemini 1.5 Flash' : 'Gemini Pro'}</span>
      ...
      {/* Quick Gemini API Key Modal */}
      {isKeyModalOpen && (
        <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-xs bg-[#0e1424] border border-indigo-500/30 rounded-2xl p-4 space-y-3 shadow-2xl">
            ...
            <p className="text-[10px] text-slate-400">
              Enter your Gemini API key to enable direct cloud generation. If left blank, the built-in smart synthesis engine will run.
            </p>
            <input
              type="password"
              value={keyInput}
              ...
              placeholder="AIzaSy..."
              className="w-full px-3 py-1.5 bg-[#060810] border border-slate-700 rounded-xl text-white font-mono text-[11px] focus:outline-none focus:border-indigo-500"
            />
            <div className="flex justify-end space-x-2 pt-1">
              <button
                onClick={() => setIsKeyModalOpen(false)}
                className="px-3 py-1 text-slate-400 hover:text-white text-[11px]"
              >
                Cancel
              </button>
              <button
                ...
                className="px-3 py-1 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[11px] rounded-xl shadow-md"
              >
                Save Key
              </button>
            </div>
                ...
                {idx > 0 && (
                  <div className="p-3.5 rounded-2xl bg-[#0f1422] dark:bg-[#0f1422] light:bg-white border border-slate-800 dark:border-slate-800 light:border-slate-200 text-slate-200 dark:text-slate-200 light:text-slate-800 space-y-2.5 shadow-sm text-xs leading-relaxed relative group">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">{renderFormattedContent(msg.content)}</div>
                      <button
